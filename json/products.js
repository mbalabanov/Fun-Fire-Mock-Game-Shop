let productsdata = JSON.parse(`{
    "products": [
       {
         "id": "1",
         "name": "Pokemon Go",
         "description": "Vestibulum id ligula porta felis euismod semper. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product1.jpg",
         "price": "20"
       },
       {
         "id": "2",
         "name": "Snooker",
         "description": "Donec sed odio dui. Praesent commodo cursus magna, vel scelerisque nisl consectetur et.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product2.jpg",
         "price": "20"

       },
       {
         "id": "3",
         "name": "Cluedo",
         "description": "Maecenas sed diam eget risus varius blandit sit amet non magna. Etiam porta sem malesuada magna mollis euismod.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product3.jpg",
         "price": "20"
       },
       {
         "id": "4",
         "name": "Virtual Reality",
         "description": "Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product4.jpg",
         "price": "20"
       },
       {
         "id": "5",
         "name": "War",
         "description": "Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum. Etiam porta sem malesuada magna mollis euismod.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product5.jpg",
         "price": "20"
       },
       {
         "id": "6",
         "name": "Minecraft",
         "description": "Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product6.jpg",
         "price": "20"
       },
       {
         "id": "7",
         "name": "Football",
         "description": "Donec sed odio dui. Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh, ut fermentum massa justo sit amet risus.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product7.jpg",
         "price": "20"
       },
       {
         "id": "8",
         "name": "Space Dance",
         "description": "Nullam id dolor id nibh ultricies vehicula ut id elit. Integer posuere erat a ante venenatis dapibus posuere velit aliquet.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product8.jpg",
         "price": "20"
       },
       {
         "id": "9",
         "name": "Chess",
         "description": "Maecenas faucibus mollis interdum. Curabitur blandit tempus porttitor. Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum.",
         "info": "Lorem Ipsum Dolor",
         "image": "img/products/product9.jpg",
         "price": "20"
       }
    ]
 }`)